...
# Locate project
PROJECT="uuhappy.Desktop/uuhappy.Desktop.csproj"
if [[ ! -f "$PROJECT" ]]; then
  PROJECT="$(find . -maxdepth 3 -name 'uuhappy.Desktop.csproj' | head -n1 || true)"
fi
[[ -f "$PROJECT" ]] || { echo "uuhappy.Desktop.csproj not found"; exit 1; }
...
choose_channel() {
  ...
  if [[ -t 0 ]]; then
    echo "[?] Choose uuhappy release channel:" >&2
...
get_latest_tag_latest() {
  curl -fsSL "https://api.github.com/repos/decardlabs/uuhappy/releases/latest" \
    | jq -re '.tag_name' \
    | sed 's/^v//'
}

get_latest_tag_prerelease() {
  curl -fsSL "https://api.github.com/repos/decardlabs/uuhappy/releases?per_page=20" \
...
# Prefer the prebuilt uuhappy bundle; then unify geo layout
download_uuhappy_bundle() {
  local outroot="$1" rid="$2"
  local url=""
  if [[ "$rid" == "linux-arm64" ]]; then
    url="https://raw.githubusercontent.com/decardlabs/uuhappy-core-bin/refs/heads/master/uuhappy-linux-arm64.zip"
  else
    url="https://raw.githubusercontent.com/decardlabs/uuhappy-core-bin/refs/heads/master/uuhappy-linux-64.zip"
  fi
  echo "[+] Try uuhappy bundle archive: $url"
  local tmp zipname
  tmp="$(mktemp -d)"; zipname="$tmp/uuhappy.zip"
  ...
  rm -f "$outroot/uuhappy.zip" 2>/dev/null || true
  find "$outroot" -type d -name "mihomo" -prune -exec rm -rf {} + 2>/dev/null || true

  local nested_dir
  nested_dir="$(find "$outroot" -maxdepth 1 -type d -name 'uuhappy-linux-*' | head -n1 || true)"
...
build_for_arch() {
  ...
  ICON_CANDIDATE="$PROJECT_DIR/uuhappy.png"
  [[ -f "$ICON_CANDIDATE" ]] || { echo "Required icon not found: $ICON_CANDIDATE"; return 1; }
  cp "$ICON_CANDIDATE" "$WORKDIR/$PKGROOT/uuhappy.png"
  ...
  if [[ "$FORCE_NETCORE" -eq 0 ]]; then
    if download_uuhappy_bundle "$WORKDIR/$PKGROOT" "$RID_DIR"; then
      echo "[*] Using uuhappy bundle archive."
    else
  ...
  # SPEC
  local SPECFILE="$SPECDIR/uuhappy.spec"
  mkdir -p "$SPECDIR"
  cat > "$SPECFILE" <<'SPEC'
...
Version:        __VERSION__
Release:        1%{?dist}
Summary:        uuhappy (Avalonia) GUI client for Linux (x86_64/aarch64)
License:        GPL-3.0-only
URL:            https://github.com/decardlabs/uuhappy
BugURL:         https://github.com/decardlabs/uuhappy/issues
...
%description
uuhappy Linux for Red Hat Enterprise Linux
Support vless / vmess / Trojan / http / socks / Anytls / Hysteria2 / Shadowsocks / tuic / WireGuard
Support Red Hat Enterprise Linux / Fedora Linux / Rocky Linux / AlmaLinux / CentOS
For more information, Please visit our website
https://github.com/decardlabs/uuhappy
...
%install
install -dm0755 %{buildroot}/opt/uuhappy
cp -a * %{buildroot}/opt/uuhappy/

# Normalize permissions
find %{buildroot}/opt/uuhappy -type d -exec chmod 0755 {} +
find %{buildroot}/opt/uuhappy -type f -exec chmod 0644 {} +
[ -f %{buildroot}/opt/uuhappy/uuhappy ] && chmod 0755 %{buildroot}/opt/uuhappy/uuhappy || :

# Launcher (prefer native ELF first, then DLL fallback)
install -dm0755 %{buildroot}%{_bindir}
install -m0755 /dev/stdin %{buildroot}%{_bindir}/uuhappy << 'EOF'
#!/usr/bin/bash
set -euo pipefail
DIR="/opt/uuhappy"

# Prefer native apphost
if [[ -x "/uuhappy" ]]; then exec "/uuhappy" "$@"; fi

# DLL fallback
for dll in uuhappy.Desktop.dll uuhappy.dll; do
  if [[ -f "$DIR/$dll" ]]; then exec /usr/bin/dotnet "$DIR/$dll" "$@"; fi
done

echo "uuhappy launcher: no executable found in $DIR" >&2
ls -l "$DIR" >&2 || true
exit 1
EOF
...
install -m0644 /dev/stdin %{buildroot}%{_datadir}/applications/uuhappy.desktop << 'EOF'
[Desktop Entry]
Type=Application
Name=uuhappy
Comment=uuhappy for Red Hat Enterprise Linux
Exec=uuhappy
Icon=uuhappy
Terminal=false
Categories=Network;
...
install -m0644 %{_builddir}/__PKGROOT__/uuhappy.png %{buildroot}%{_datadir}/icons/hicolor/256x256/apps/uuhappy.png
...
%{_bindir}/uuhappy
/opt/uuhappy
%{_datadir}/applications/uuhappy.desktop
%{_datadir}/icons/hicolor/256x256/apps/uuhappy.png
SPEC
  ...
  local f
  for f in "${TOPDIR}/RPMS/${archdir}/uuhappy-${VERSION}-1"*.rpm; do
    [[ -e "$f" ]] || continue
    echo "  $f"
    BUILT_RPMS+=("$f")
  done
