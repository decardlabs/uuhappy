...
public class CheckUpdateViewModel : MyReactiveObject
{
    ...
    private readonly string _uuhappy = ECoreType.uuhappy.ToString();
    ...
    public CheckUpdateViewModel(Func<EViewAction, object?, Task<bool>>? updateView)
    {
        ...
        CheckUpdateCmd.ThrownExceptions.Subscribe(ex =>
        {
            ...
            _ = UpdateView(_uuhappy, ex.Message);
        });
    ...
    private void RefreshCheckUpdateItems()
    {
        ...
        {
            CheckUpdateModels.Add(GetCheckUpdateModel(_uuhappy));
    ...
    {
        if (coreType == _uuhappy && Utils.IsPackagedInstall())
        {
            return new()
            {
                IsSelected = false,
    ...
    private async Task CheckUpdateTask()
    {
        ...
        for (var k = CheckUpdateModels.Count - 1; k >= 0; k--)
        {
            ...
            else if (item.CoreType == _uuhappy)
            {
                if (Utils.IsPackagedInstall())
                {
                    await UpdateView(_uuhappy, "Not Support");
                    continue;
                }
    ...
    private async Task CheckUpdateN(bool preRelease)
    {
        async Task _updateUI(bool success, string msg)
        {
            await UpdateView(_uuhappy, msg);
            if (success)
            {
                await UpdateView(_uuhappy, ResUI.OperationSuccess);
                UpdatedPlusPlus(_uuhappy, msg);
            }
        }
        await new UpdateService(_config, _updateUI).CheckUpdateGuiN(preRelease)
            .ContinueWith(t => UpdatedPlusPlus(_uuhappy, ""));
    }
    ...
    private async Task UpdateFinished()
    {
        if (_lstUpdated.Count > 0 && _lstUpdated.Count(x => x.IsFinished == true) == _lstUpdated.Count)
        {
            ...
            if (_lstUpdated.Any(x => x.CoreType == _uuhappy && x.IsFinished == true))
            {
    ...
    private async Task UpgradeN()
    {
        try
        {
            var fileName = _lstUpdated.FirstOrDefault(x => x.CoreType == _uuhappy)?.FileName;
            if (fileName.IsNullOrEmpty())
            {
                return;
            }
            ...
            {
                await UpdateView(_uuhappy, ResUI.UpgradeAppNotExistTip);
        ...
        {
            await UpdateView(_uuhappy, ex.Message);
        }
    }
