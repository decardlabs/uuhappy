...
public class UpdateService(Config config, Func<bool, string, Task> updateFunc)
{
    ...
    public async Task CheckUpdateGuiN(bool preRelease)
    {
        ...
        await UpdateFunc(false, string.Format(ResUI.MsgStartUpdating, ECoreType.uuhappy));
        var result = await CheckUpdateAsync(downloadHandle, ECoreType.uuhappy, preRelease);
        if (result.Success)
        {
            await UpdateFunc(false, string.Format(ResUI.MsgParsingSuccessfully, ECoreType.uuhappy));
    ...
    private async Task<string?> GetUrlFromCore(CoreInfo? coreInfo)
    {
        if (Utils.IsWindows())
        {
            ...
            if (coreInfo?.CoreType != ECoreType.uuhappy)
            {
                return url;
            }
