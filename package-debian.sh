...
build_for_arch() {
  ...
  cp -a "$PUBDIR/." "$STAGE/opt/uuhappy/"

  local ICON_CANDIDATE
  PROJECT_DIR="$(cd "$(dirname "$PROJECT")" && pwd)"
  ICON_CANDIDATE="$PROJECT_DIR/uuhappy.png"
  [[ -f "$ICON_CANDIDATE" ]] && cp "$ICON_CANDIDATE" "$STAGE/usr/share/icons/hicolor/256x256/apps/uuhappy.png" || true

  mkdir -p "$STAGE/opt/uuhappy/bin/xray" "$STAGE/opt/uuhappy/bin/sing_box"
  ...
  if [[ "$FORCE_NETCORE" -eq 0 ]]; then
    if download_uuhappy_bundle "$STAGE/opt/uuhappy" "$RID_DIR"; then
      echo "[*] Using uuhappy bundle bin assets."
    else
      echo "[*] Bundle failed, fallback to separate core + rules."
      fetch_separate_cores_and_rules "$STAGE/opt/uuhappy"
    fi
  else
  ...
  # Wrapper
  install -m 755 /dev/stdin "$STAGE/usr/bin/uuhappy" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
DIR="/opt/uuhappy"
cd "$DIR"
...
Section: net
Priority: optional
Maintainer: decardlabs <noreply@github.com>
...
EOF
  ...
  mapfile -t ELF_FILES < <(
    find "$STAGE/opt/uuhappy" -type f \( -name "*.so*" -o -perm -111 \) ! -name 'libcoreclrtraceptprovider.so'
  )
  ...
  # Desktop file
  install -m 644 /dev/stdin "$STAGE/usr/share/applications/uuhappy.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=uuhappy
Comment=uuhappy for Debian GNU Linux
Exec=uuhappy
Icon=uuhappy
Terminal=false
Categories=Network;
...
Architecture: ${deb_arch}
Maintainer: decardlabs <noreply@github.com>
Homepage: https://github.com/decardlabs/uuhappy
Section: net
Priority: optional
Depends: ${FINAL_DEPENDS}
Description: uuhappy (Avalonia) GUI client for Linux
...
EOF
  ...
  find "$STAGE/opt/uuhappy" -type d -exec chmod 0755 {} +
  find "$STAGE/opt/uuhappy" -type f -exec chmod 0644 {} +
  [[ -f "$STAGE/opt/uuhappy/uuhappy" ]] && chmod 0755 "$STAGE/opt/uuhappy/uuhappy" || true
  
  local deb_out
  deb_out="$OUTPUT_DIR/uuhappy_${VERSION}_${deb_arch}.deb"
